...
frappe.listview_settings['Attendance'] = {
	...
	get_indicator: function (doc) {
		if (["Present", "Work From Home", "Rest Day"].includes(doc.status)) {
	...
	onload: function(list_view) {
		...
		if (frappe.boot.attendance_import_enabled) {
         ...
         list_view.page.add_inner_button(__("Recalculate Attendance"), async function () {
			frappe.confirm(__('Are you sure you want to recalculate attendance?'), function () {
			    frappe.call({
			        method: "recalculate_attendance",
			        callback: function(response) {
                              if (response.message) {
                                frappe.msgprint(response.message); // Display success or error message
                              } else {
                                frappe.msgprint("Server script execution complete."); // Log for debugging
                              }
                            }  
			    }
			        )
