def recalculate_salary_slip(salary_slip):
    ...
    # frappe.msgprint(f"salary_slip.basic_allowance_amount: {salary_slip.basic_allowance_amount}")
    # frappe.msgprint(f"daily_hours: {daily_hours}")
    ...
    frappe.msgprint(f"total_late_in: {total_late_in} | total_undertime: {total_undertime}")
    ...
    # frappe.msgprint(f"legal_holidays: {legal_holidays}")
    ...
    # frappe.msgprint(f"total_leave: {total_leave}")
    # frappe.msgprint(f"total_absences: {total_absences}")
    # frappe.msgprint(f"total_present: {total_present}")
    ...
    # frappe.msgprint(f"total_hours: {total_hours}")
    ...
    # frappe.msgprint(f"gross_pay_for_slip: {gross_pay_for_slip}")
    ...
    frappe.msgprint(f"posting date day updated {cut_off_day}")    
    ...
    if cut_off_day not in [27, 28, 29, 30, 31]:
        ...
        if previous_slip:
            ...
            frappe.msgprint(f"prev: {previous_cut_off_phic}")
            frappe.msgprint(f"calculated_phic_contribution based on prev: {calculated_phic_contribution}")
            ...
            for row in previous_slip_doc.get("statistical_deductions"):    
                if row.salary_component == "PH - SSS Employee Compensation":
                    # frappe.msgprint(f" previous_cut_off_emp_com {previous_cut_off_emp_com}")
